...
let canvas, context, input, mouse_status, mx, my, crop_x, crop_y, crop_scale
...
const calc = () => {
    if (crop_width > (canvas_width - (crop_border*2)) || crop_height > (canvas_height - (crop_border*2))) {
        var wRatio = (canvas_width - (crop_border*2))/crop_width
        var hRatio = (canvas_height - (crop_border*2))/crop_height
        crop_scale = (wRatio > hRatio) ? hRatio : wRatio
    } else {
        crop_scale = 1
    }
    console.log(crop_scale)
    crop_x = ((canvas_width - (crop_width*crop_scale))/2)
    crop_y = ((canvas_height - (crop_height*crop_scale))/2)
}
...
const handleImageReady = () => {
    const wRatio = (crop_width * crop_scale)/image.width
    const hRatio = (crop_height * crop_scale)/image.height
    if (wRatio > hRatio) {
        image_scale = wRatio
    ...
    image_x = (( (crop_width * crop_scale) - image_width) / 2) + crop_x
    image_y = (( (crop_height * crop_scale) - image_height) / 2) + crop_y
    image_status = true
    draw()
}
...
const paintBox = () => {
    ...
    context.beginPath()
    context.rect(crop_x, crop_y, crop_width*crop_scale, crop_height*crop_scale)
...
const save = () => {
    ...
    c.getContext('2d').drawImage(image, (image_x - crop_x) / crop_scale, (image_y - crop_y) / crop_scale, image_width / crop_scale, image_height / crop_scale)
    c.toBlob((blob) => {
